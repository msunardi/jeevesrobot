 ...
 */
#include "guidebotNavigation.h"				// See notes in header
...
*/
		...
		*/
		...
		if(strcmp(ODO_PORT_NAME,LRF_PORT_NAME) == 0){		//if lrf and odometry are on the same arduino port
		...
		if(strcmp(ODO_PORT_NAME,MOTOR_PORT_NAME) == 0){		//if motor is also on the same port
		...
		if (thrPar.quit.get())						//Check error condition
			return 0;

        c = mrpt::system::os::getch();          //clear input
		// --------------------------------------------------------
		// Menu Start
		// --------------------------------------------------------
		while (run)
		{
            if (show_menu)
			{
				cout << endl << "Press the key for your option:" << endl;
				...
				cout << " l		: Wireless Localization: " << endl;
				cout << " n		: Network communication mode" << endl;
				cout << " Q     : Quit" << endl;
			}
			c = mrpt::system::os::getch();             //get input
			cout << c;
			...
			if (c=='l'){
                WIFILocalize(robot,thrPar);
			}
			if (c=='A'){
                turn(90,thrPar);
			}

			if (c=='w' || c=='s' || c=='a' || c=='d' || c=='x') // increase or decrease current linear velocity
			{
           		move(c,thrPar);
			}
			...
			if (c=='o')  // get current odometry reading
			{
				...
				//printf("***x = %d, y = %d, phi = %d\n",odo.x(),odo.y(),odo.phi());
				cout << "Odometry: " << odo << " v: " << v << " w: " << RAD2DEG(w) << " leBft: " << left_ticks << " right: " << right_ticks << endl;
				counter = 0;
			}
...
/*
 * @Description
 * This thread does montecarlo localization and put the Particle Density Function (pdf)
 * into ThreadRobotParam.
...
/*
	if ( !guidebotConfFile.read_bool("LocalizationParams","init_PDF_mode",false, true) )
...
void thread_display(TThreadRobotParam &p)
{
	...
	{
        do{
			p.usingLRF.set(true);				// lock mutex
        ...
        */
		...
		if(p.displayNewPdf.get() )
		{
			...
			//obj4->setPose( tempPose );	//tempPdf->getMostLikelyParticle() ); //jkw
		...
		*/
		...
		p.usingLRF.set(false);				//Unlock mutex
	}
...
int getNextObservation(CObservation2DRangeScan & out_obs, bool there_is, bool hard_error, int fd, TThreadRobotParam &thrPar)
{
	...
	do{
		thrPar.gettingLRF.set(true);	//lock mutex
	}while(thrPar.usingLRF.get());
	...
	thrPar.gettingLRF.set(false);							// unlock mutex
	//cout<<thrPar.gettingLRF.get()<<"LRF VALUE"<<endl;
...
int clientCommunication()
{
    ...
    int holdScanf;      //holds scanf return
	//keep communicating with server
	while(1)
	{
		printf("Enter message : ");
		holdScanf = scanf("%s" , message);
...
int parsesequence(TThreadRobotParam &thrPar)
{
    ...
    if (file != NULL) {
        ...
        garbage = fgets(line, sizeof(line), file);
...
void sequence(char sequencechar, TThreadRobotParam &thrPar)
{
    ...
    int startOdo, curOdo, turnTime, forwardTime;
    float front2, side2, frontStart;
    CPose2D 	odo;
	...
	frontStart = thrPar.front_wall.get();
...
/*
    do{
		...
		//fixOdometry( odo, thrPar.odometryOffset.get() );
		curOdo = odo.phi();
		cout << "test param odo " << curOdo << " " << startOdo << endl;
		sleep(turnTime);
    }
    while(curOdo - 90 > startOdo);
*/
	...
	/*CPose2D 	odo;
			    ...
			    cout << "test right " << thrPar.front_wall.get() << endl << endl;
                ...
                old_phi = odo.phi();
			    setVelocities( 0, 1, thrPar );
			    while(abs(old_phi - odo.phi()) <= M_PI/2)
			    {
			        cout << " M_PI/2 = " << M_PI/2 << endl;
			        cout <<" Diff = " << abs(old_phi - odo.phi()) << endl;
			        //sleep(1000);
			        //if ()
			        getOdometry( odo, odo_fd, thrPar );
			        fixOdometry( odo, thrPar.odometryOffset.get() );
...
void move(char direction, TThreadRobotParam &thrPar){
	double cur_v, cur_w;
	if (direction=='w' || direction=='s') // increase or decrease current linear velocity
	{
		thrPar.isMoving.set(true);
		thrPar.goForward.set(true);
		thrPar.goRight.set(false);
		if (direction=='w') cur_v = 1;
		if (direction=='s') cur_v = -1;
		setVelocities( cur_v, 0, thrPar);
		sleep(100);
	}
	if (direction=='a' || direction=='d')  // increase or decrease current angular velocity
	{
		thrPar.isMoving.set(true);
		thrPar.goForward.set(false);
		thrPar.goRight.set(true);
		if (direction=='a') cur_w = -1;
		if (direction=='d') cur_w = 1;
		setVelocities( 0, cur_w, thrPar );
		sleep(100);
	}
	if (direction=='x')  // stop, set current linear and angular velocities to 0
	{
		cur_v = 0;
		cur_w = 0;
		thrPar.goForward.set(false);
...
void setVelocities(int linear, int angular, TThreadRobotParam &thrPar)
{
	...
	if(linear == 0 && angular == 0)
	{
		velocityCommand[0] = 'x';
		/* read up to 128 bytes from the fd */
		wb = write(velocity_fd,velocityCommand,1);
		sleep(100);
...
void WIFILocalize(CActivMediaRobotBase &robot,TThreadRobotParam &thrPar){
    WirelessLocalizer wifi;

    wifi.Localize();

    double newX, newY, newPhi;

    newPhi = thrPar.currentOdo.get().phi();
    newX = 9;//newX.GetCoordinateX();
    newY = -109;//newY.GetCoordinateY();

    CPose2D startOdo;
    getOdometry( startOdo, thrPar.odo_fd.get(), thrPar);
    startOdo.x(0);
    startOdo.y(0);
    //CPose2D startOdo(newX,newY,DEG2RAD(newPhi));
    CPose2D newOffset(newX,newY,DEG2RAD(newPhi) - startOdo.phi() );
