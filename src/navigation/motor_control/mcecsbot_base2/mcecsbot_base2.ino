...
void detected_obstacle(){
  ...
  /*if(Wire.available())
  {
   ...
   //if (avoid_count > avoidstepsize) {
	 //		avoid_count = avoidstepsize;
   //   shiftArray(avoid_steps, avoidstepsize);
   //}
   if ((sonar_number == 12) || (sonar_number == 11)) {
     // OBSTACLE: FORWARD RIGHT/AVOID BACKWARD/TURN LEFT
     //new_movement = LEFT;
     avoid_direction = 1;
     avoid(BACKWARD);
     avoid(LEFT);
   } else if ((sonar_number == 7) || (sonar_number == 6) || (sonar_number == 5)) {
     // OBSTACLE: FORWARD LEFT/AVOID BACKWARD/TURN RIGHT
     avoid_direction = 0;
     avoid(BACKWARD);
     avoid(RIGHT);
   } else if ((sonar_number == 2) || (sonar_number == 3) || (sonar_number == 4)) {
     // OBSTACLE: BACK LEFT/AVOID FORWARD/TURN LEFT
     avoid_direction = 2;
     avoid(FORWARD);
     avoid(LEFT);
   } else if ((sonar_number == 8) || (sonar_number == 9) || (sonar_number == 10)) {
     // OBSTACLE: BACK RIGHT/AVOID FORWARD/TURN RIGHT
     avoid_direction = 3;
     avoid(FORWARD);
		 avoid(RIGHT);
   } else {
   ...
   //avoid_steps[avoid_count] = avoid_direction;
     //delay(AVOID_DELAY);
     //new_movement = STOP;
     //doMove();
   delay(AVOID_DELAY);
