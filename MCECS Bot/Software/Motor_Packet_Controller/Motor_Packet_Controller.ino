    ...
    if(bytein0 == 0xFF && bytein1 == 0xFF)  
    {
      ...
      switch (datarray[0]) {
        ...
        case 1:
          ...
          // motor 1 command
          roboclaw.SpeedDistanceM1(0x80,Mspeed,datarray[2]);
          /*uint8_t M1depth1,M1depth2;
          do{
            displayspeed();
            roboclaw.ReadBuffers(0x80,M1depth1,M1depth2);
          }while(M1depth1);
*/

          // motor 3 command
          roboclaw.SpeedDistanceM1(0x81,Mspeed,datarray[4]);
          /*uint8_t M3depth1,M3depth2;
          do{
            displayspeed();
            roboclaw.ReadBuffers(0x81,M3depth1,M3depth2);
          }while(M3depth1);         
          */
      }
