# Maisee Brown
# 10/29/2012
# LiftHand.py (Arm Movements Collection 6/6)
...
# Set mode to pose mode
  ...
  #  counter = 1
    ...
    for k in range(0, WristLiftMin-WristLiftStep, -WristLiftStep):
        # set new rotation angles
        PoseHand.rotation_euler = (math.radians(k), math.radians(0), math.radians(0))
        ...
        bpy.ops.anim.change_frame(frame = counter)
    for j in range(WristLiftMin, WristLiftMax+WristLiftStep, WristLiftStep):
        # set new rotation angles
        PoseHand.rotation_euler = (math.radians(j), math.radians(0), math.radians(0))
        #insert key frame so we can mark this position in our animation
        PoseHand.keyframe_insert(data_path="rotation_euler")
        # increment counter so the animation progresses in time
        counter = counter + counterIncrement
        ...
        bpy.ops.anim.change_frame(frame = counter)
    for j in range(WristLiftMax, 0-WristLiftStep, -WristLiftStep):
        # set new rotation angles
        PoseHand.rotation_euler = (math.radians(j), math.radians(0), math.radians(0))
        #insert key frame so we can mark this position in our animation
        PoseHand.keyframe_insert(data_path="rotation_euler")
