# Maisee Brown
# 10/29/2012
# LiftShoulder.py (Arm Movements Collection 4/6)
...
# Set mode to pose mode
  ...
  #  counter = 1
    ...
    for k in range(ElbowLiftMax, ElbowLiftMin-ElbowLiftStep, -ElbowLiftStep):
        # set new rotation angles
        PoseLowerArm.rotation_euler = (math.radians(k), math.radians(0), math.radians(0))
        ...
        bpy.ops.anim.change_frame(frame = counter)
    for j in range(ElbowLiftMin, ElbowLiftMax+ElbowLiftStep, ElbowLiftStep):
        # set new rotation angles
        PoseLowerArm.rotation_euler = (math.radians(j), math.radians(0), math.radians(0))
        #insert key frame so we can mark this position in our animation
        PoseLowerArm.keyframe_insert(data_path="rotation_euler")
        # increment counter so the animation progresses in time
        counter = counter + counterIncrement
