...
- (void)dctInternal_setupInternals {
	self.numberOfRows = DTGridViewInvalid;
...
- (void)addCellWithInfo:(NSObject<DTGridViewCellInfoProtocol> *)info {
    ...
    DTGridViewCell *cell = [[DTGridViewCell alloc]initWithFrame:CGRectZero];
    cell = [self findViewForRow:info.yPosition column:info.xPosition];
	...
	[[self.gridCells objectAtIndex:info.yPosition] replaceObjectAtIndex:info.xPosition withObject:cell];
...
- (void)removeCellWithInfo:(DTGridViewCellInfo *)info {
	...
	if (info.yPosition > [self.gridCells count]) return;
	
	NSMutableArray *row = [self.gridCells objectAtIndex:info.yPosition];
...
- (void)initialiseViews {
	...
	for (NSUInteger i = 0; i < [self.gridCells count]; i++) {
		
		NSMutableArray *row = [self.gridCells objectAtIndex:i];
...
/*- (NSInteger)numberOfRows {
	if (self.numberOfRows == DTGridViewInvalid) {
